...
ggsave("plots/Supplementary/Sup3_TBNC.pdf",width = 7, height = 7.5)
...
##Supplementary plot 5 co-mutation
```{r}
#by gene
...
stat.test <- hrd_plot_df %>%
  ...
  mutate(p.adj = p.adjust(p),
         p.adj = case_when(p.adj >=0.05 ~ "ns",
                           p.adj <0.05 & p.adj >=0.0001 ~ as.character(round(p.adj, 3)),
                           p.adj <0.0001 ~ "<0.0001")) %>%
  ungroup() %>%
...
```
##Supplementary plot 6 HRD breakdown
```{r}
...
olap_all_tall_summary <- olap_all_tall %>%
  ...
  mutate(group = case_when(cell_line_display_name == "SNU601" ~ "Homozygous RAD51Cme",
  ...
  mutate(group = case_when(HRD >=42 & group == "No Methylation" ~ "No Methylation (HRD Score >= 42)",
...
olab_plot <- olap_all_tall_summary %>%
  ...
  geom_line(data = all_fits %>%filter(group == "Homozygous RAD51Cme"), aes(y = p), size = 1) +
  geom_point(data = .%>% filter(group == "Homozygous RAD51Cme")) +
  
  scale_colour_manual(values = c("Homozygous RAD51Cme" = "#165e91",
                                 "Heterozygous RAD51Cme" = "#7BB2DA",
                                 "No Methylation (HRD Score < 42)" = "lightgrey",
                                 "No Methylation (HRD Score >= 42)" = "#CEB1BE",
                                 "SNU520"= "black"), "RAD51C status") +
  xlim(1,150) +
  ylim(0.2,1.00) +
...
pdf("Main_Figure_7_olaparib_STAD_dose_curve.pdf", width = 6, height = 6)
olab_plot
dev.off()
