...
col = list(alt = c("MUT" = "#EE8A8A", "Conseq" = "#616161", "NA" = "white"),
           ...
           `Global Methylation` = c("Low" = "#D6F8D6", "High" = "#7FC6A4", "Intermediate" = "#B0C7BD", "NA" = "white"),
...
write.csv(beta_sub_meth %>%
  ...
  filter(Sample_Group == "Primary Tumour", gene_promo_200=="BRCA1", Sample_Name %in% Deduplicated_beta$Sample_Name) %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, CpG, beta, Dataset = Project) %>%
  distinct() %>%
  pivot_wider(names_from = CpG, values_from = beta), "data/source_data/Source_Data_1_primary_tumour_BRCA1_beta.csv", quote = F, row.names = F)

write.csv(beta_sub_meth %>%
  mutate(Sample_Group = gsub("Tumor", "Tumour", Sample_Group)) %>%
  filter(Sample_Group == "Solid Tissue Normal",gene_promo_200 =="BRCA1", Sample_Name %in% Deduplicated_beta$Sample_Name) %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, CpG, beta, Dataset = Project) %>%
  distinct() %>%
  pivot_wider(names_from = CpG, values_from = beta), "data/source_data/Source_Data_1_normal_tissue_BRCA1_beta.csv", quote = F, row.names = F)

write.csv(beta_sub_meth %>%
  mutate(Sample_Group = gsub("Tumor", "Tumour", Sample_Group)) %>%
  filter(Sample_Group == "Primary Tumour",gene_promo_200 =="RAD51C", Sample_Name %in% Deduplicated_beta$Sample_Name) %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, CpG, beta, Dataset = Project) %>%
  distinct() %>%
  pivot_wider(names_from = CpG, values_from = beta), "data/source_data/Source_Data_1_primary_tumour_RAD51C_beta.csv", quote = F, row.names = F)

write.csv(beta_sub_meth %>%
  mutate(Sample_Group = gsub("Tumor", "Tumour", Sample_Group)) %>%
  filter(Sample_Group == "Solid Tissue Normal",gene_promo_200 =="RAD51C", Sample_Name %in% Deduplicated_beta$Sample_Name) %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, CpG, beta, Dataset = Project) %>%
  distinct() %>%
  pivot_wider(names_from = CpG, values_from = beta), "data/source_data/Source_Data_1_normal_tissue_RAD51C_beta.csv", quote = F, row.names = F)

write.csv(beta_sub %>%
  mutate(Sample_Group = gsub("Tumor", "Tumour", Sample_Group)) %>%
  filter(Sample_Group == "Primary Tumour",gene_promo_200 =="BRCA2", Sample_Name %in% Deduplicated_beta$Sample_Name) %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, CpG, beta, Dataset = Project) %>%
  distinct() %>%
  pivot_wider(names_from = CpG, values_from = beta), "data/source_data/Source_Data_1_primary_tumour_BRCA2_beta.csv", quote = F, row.names = F)

write.csv(beta_sub %>%
  mutate(Sample_Group = gsub("Tumor", "Tumour", Sample_Group)) %>%
  filter(Sample_Group == "Solid Tissue Normal",gene_promo_200 =="BRCA2", Sample_Name %in% Deduplicated_beta$Sample_Name) %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, CpG, beta, Dataset = Project) %>%
  distinct() %>%
  pivot_wider(names_from = CpG, values_from = beta), "data/source_data/Source_Data_1_normal_tissue_BRCA2_beta.csv", quote = F, row.names = F)

write.csv(beta_sub %>%
  mutate(Sample_Group = gsub("Tumor", "Tumour", Sample_Group)) %>%
  filter(Sample_Group == "Primary Tumour",gene_promo_200 =="BRIP1", Sample_Name %in% Deduplicated_beta$Sample_Name) %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, CpG, beta, Dataset = Project) %>%
  distinct() %>%
  pivot_wider(names_from = CpG, values_from = beta), "data/source_data/Source_Data_1_primary_tumour_BRIP1_beta.csv", quote = F, row.names = F)

write.csv(beta_sub %>%
  mutate(Sample_Group = gsub("Tumor", "Tumour", Sample_Group)) %>%
  filter(Sample_Group == "Solid Tissue Normal",gene_promo_200 =="BRIP1", Sample_Name %in% Deduplicated_beta$Sample_Name) %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, CpG, beta, Dataset = Project) %>%
  distinct() %>%
  pivot_wider(names_from = CpG, values_from = beta), "data/source_data/Source_Data_1_normal_tissue_BRIP1_beta.csv", quote = F, row.names = F)

```
##Source data 2 - subtypes
```{r}
 write.csv(TCGA_subtype %>%
             mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8 ), substr(Sample.ID, 1, 12))) %>%
             distinct() %>%
             dplyr::select(Dataset = Project, Methylation, CaseID, Global_Methylation = Cluster, Histological_Subtype = `Cancer Type Detailed`, Molecular_Subtype = Subtype)
           ,paste0("data/source_data/Source_Data_2_Subtype.csv") , quote = F, row.names = F)
...
Molecular_subtype_table$fisher.adj <- p.adjust(Molecular_subtype_table$fisher)
write.csv(Molecular_subtype_table, "data/supplementary_data/Table_S2_Subtype_fishers_molecular.csv", quote = F, row.names = F)
...
Histo_subtype_table$fisher.adj <- p.adjust(Histo_subtype_table$fisher)
...
demograph_table_meth <- fread("data/source_data/Source_Data_4_demograph_table_meth.csv") %>%
...
```
##Supplementary Table ? - ancestry fisher
```{r}
#sex
ances_table <-  demograph_table_meth  %>%
  filter(!is.na(consensus_ancestry)) %>%
  ...
  ungroup() %>%
  filter(Methylation != "No") %>%
  mutate(Meth_Ances = n, Meth_NOT_Ances = Methylation_total - n,
         Unmeth_Ances = ances_total - n, Unmeth_Not_Ances = Cancer_total - ances_total - (Methylation_total - n)) %>%
  dplyr::select(-n, -ances_total, - Methylation_total, - Cancer_total) %>%
  distinct()

ances_fisher <- ances_table %>%
  group_by(Dataset, Methylation, consensus_ancestry) %>%
  group_map(~ fisher.test(x = t(matrix(unlist(.x),byrow = T, nrow = 2))))

ances_table$fisher <- unlist(lapply(ances_fisher, function(x) x$p.value))

ances_table<-  ances_table%>%
  group_by(Dataset) %>%
  mutate(fisher.adj = p.adjust(fisher))
```
##Source data 3
...
write.csv(demograph_table_meth %>%
            dplyr::rename(SNP_ancestry = consensus_ancestry), "data/source_data/Source_Data_3_demograph_table_meth.csv", quote = F, row.names = F)
...
cimp_others <- TCGA_subtype %>%
  ...
  theme_bw()+
  scale_fill_manual(values = col$`Global Methylation`, "Global Methylation Cluster")+
  theme(legend.position = "bottom", axis.title.x = element_blank())+ 
  scale_y_continuous(label = scales::percent)+
  ylab("Percent of Patient")+
  scale_x_discrete(labels = function(x) str_wrap(x, width = 5))
...
##Source data 4 - cimp
```{r}
cimp_stad_lgg_table<- TCGA_subtype %>%
  group_by(Project, Methylation) %>%
  filter(n()>2) %>%
  ungroup() %>%
  filter(Project %in% c("STAD", "LGG")) %>%
...
cimp_others_table <- TCGA_subtype %>%
   group_by(Project, Methylation) %>%
  filter(n()>2) %>%
  ungroup() %>%
   filter(!Project %in% c("STAD", "LGG")) %>%
...
write.csv(rbind(cimp_stad_lgg_table, cimp_others_table), "data/source_data/Source_Data_4_cimp.csv", quote = F, row.names = F)
```
...
onco <- oncoPrint(
  ...
  top_annotation =HeatmapAnnotation(
    ...
    `Global Methylation` =factor(oncoprint_matrix["cimp_label",], levels = c("High", "Intermediate", "Low", "NA")),
...
```
##Source data 5 co-mutation
```{r}
write_tsv(mut_beta_sub_meth_t_per_sample , "data/source_data/Source_Data_5_co-mutation.csv")
...
write_tsv(brca1_rad51c_unmeth_prev_table %>%
            filter(Methylated_gene == "BRCA1") %>%
             dplyr::select(Hugo_Symbol, Unmeth_Mut = Unmeth_mut, Meth_Mut = Meth_mut, Total_Unmeth = total_unmeth, Total_meth = total_meth, Prevalence_Unmethylated, Prevalence_Unmethylated, fisher_p, pval.adj), "data/supplementary_data/Table_S6_mutation_by_BRCA1.tsv")

write_tsv(brca1_rad51c_unmeth_prev_table %>%
            filter(Methylated_gene == "RAD51C") %>%
             dplyr::select(Hugo_Symbol, Unmeth_Mut = Unmeth_mut, Meth_Mut = Meth_mut, Total_Unmeth = total_unmeth, Total_meth = total_meth, Prevalence_Unmethylated, Prevalence_Unmethylated, fisher_p, pval.adj), "data/supplementary_data/Table_S6_mutation_by_RAD51C.tsv")

write_tsv(type_brca1_rad51c_prev_table %>%
            filter(Methylated_gene == "BRCA1") %>%
            dplyr::select(Dataset = Project, Hugo_Symbol, Unmeth_Mut = Unmeth_mut, Meth_Mut = Meth_mut, Total_Unmeth = total_unmeth, Total_meth = total_meth, Prevalence_Unmethylated, Prevalence_Unmethylated, fisher_p, pval.adj), "data/supplementary_data/Table_S6_mutation_by_project_BRCA1.tsv")

write_tsv(type_brca1_rad51c_prev_table %>%
            filter(Methylated_gene == "RAD51C") %>%
            dplyr::select(Dataset = Project, Hugo_Symbol, Unmeth_Mut = Unmeth_mut, Meth_Mut = Meth_mut, Total_Unmeth = total_unmeth, Total_meth = total_meth, Prevalence_Unmethylated, Prevalence_Unmethylated, fisher_p, pval.adj), "data/supplementary_data/Table_S6_mutation_by_project_RAD51C.tsv")
...
##Source data 6 - expression
```{r}
write.csv(BRCA2_mRNA_table %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, Dataset = Project, BRCA2me= Methylation, BRCA2_mRNA = mRNA_norm_counts) %>%
  distinct(), "data/source_data/Source_Data_6_BRCA2_expression.csv", quote = F, row.names = F)


write.csv(BRIP1_mRNA_table %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, Dataset = Project, BRIP1me = Methylation, BRIP1_mRNA = mRNA_norm_counts) %>%
  distinct(), "data/source_data/Source_Data_6_BRIP1_expression.csv", quote = F, row.names = F)


write.csv(PALB2_mRNA_table %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(CaseID, Dataset = Project, BRCA2me= Methylation, BRCA2_mRNA = mRNA_norm_counts) %>%
  distinct(), "data/source_data/Source_Data_6_PALB2_expression.csv", quote = F, row.names = F)


BRCA1_RAD51C_mRNA_table <- beta_sub_meth_mrna_t %>%
  filter(Project != "MESO", !is.na(mRNA_norm_counts)) %>%
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15))) %>%
  dplyr::select(gene, Project, Methylation, CaseID, mRNA_norm_counts) %>%
  distinct() %>%
  group_by(gene, Project) %>%
  filter(sum(Methylation!= "No") >0)


write.csv(BRCA1_RAD51C_mRNA_table %>%
            filter(gene == "BRCA1") %>%
  dplyr::select(CaseID, Dataset = Project, BRCA1me = Methylation, BRCA1_mRNA = mRNA_norm_counts) %>%
  distinct(), "data/supplementary_data/Source_Data_6_BRCA1_expression.csv", quote = F, row.names = F)

write.csv(BRCA1_RAD51C_mRNA_table %>%
            filter(gene == "RAD51C") %>%
  dplyr::select(CaseID, Dataset = Project, RAD51Cme = Methylation, RAD51C_mRNA = mRNA_norm_counts) %>%
  distinct(), "data/source_data/Source_Data_6_RAD51C_expression.csv", quote = F, row.names = F)
...
bar_code_h <- explore_df %>%
  filter(Methylation == "RAD51C") %>%
  mutate(TP53 = if_else(TP53 != "", 1, 0 ))%>%
  ggplot() +
  geom_point(aes(y = HRD_Score, x = 2))+
  geom_hline(aes(yintercept = HRD_Score), data = . %>%filter(TP53 == 1))+
...
fit1 <- lm(HRD_Score ~ avg_meth, data = explore_df %>%
...
```
##Supplementary plot 6 TMB MSI Aneup
```{r}
...
msi_plot <- msi_plot_df  %>%
  dplyr::select(Methylation, value, parameter_name, Methylation_N, Project) %>%
  distinct() %>%
  ggplot(aes(x = Methylation, y = value, color =Methylation))+
   geom_point(position = position_jitterdodge(jitter.width = 0.8), size = 0.8)+
  geom_boxplot(outlier.shape = NA, data = . %>% ungroup()%>% group_by(Project, Methylation, .keep = T) %>%
...
explore_df %>%
  ...
  mutate(Project = factor(Project, levels= c("OV","TGCT", "STAD", "LGG", "BC", "UCEC", "COAD", "HNSC"))) %>%
ggpairs( columns = c("HRD Score", "Expression (z-score)", "Avg Methylation"), diag = list(continuous = "blankDiag"), aes(  shape = Methylation, size = 0.8), lower = list(continuous = "points", combo= "facethist", discrete = "facetbar", na =
    "na"), upper = list(continuous =wrap("cor", method = "spearman", exact = F), combo = "facethist", discrete = "facetbar", na =
    "na"))+
  theme_bw()+
  scale_shape_manual(values = c(21))+
  scale_fill_brewer(palette="Set1",  drop=FALSE)
...
top_annotation =HeatmapAnnotation(
    ...
    `Avg Meth` = anno_points(apply((matrix(HRD_onco_mat[c("avg_meth","avg_corr_meth"),],nc =2, byrow = T)),2, as.numeric), pch = c(21,3), gp = gpar(col = c("#E69F00","#0072B2")),show_annotation_name = F),
