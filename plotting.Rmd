...
```
##Supplementary plot HRD breakdown
```{r}
...
LOH_plot <- hrd_component_df %>%
  ...
  scale_y_continuous("LOH", limits = c(0, 40))+
  scale_x_discrete(labels = function(x) str_wrap(x, width = 5))
...
ggsave("plots/Supplementary/Sup6a_HRD_breakdown.pdf", height = 10, width = 16)
```
## gLOH
```{r}
gLOH_tcga <- DDR_score %>%
  mutate(gLOH = LOH_n_seg/CNA_n_segs) %>%
...
gLOH_icgc <-Sys.glob("data/raw/ascat/AOCS*.copynumber.ascat.tsv") %>%
  ...
  group_by(Sample.ID) %>%
  summarise(total_segment = n(), n_LOH= sum(LOH==TRUE)) %>%
  ungroup() %>% 
  mutate(Sample.ID =substr(Sample.ID, 1, 8),
         gLOH = n_LOH / total_segment) %>%
