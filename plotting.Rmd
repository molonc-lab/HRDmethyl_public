...
# 
# beta_sub <- beta_sub %>%
#   filter(gene %in% c("BRCA1", "BRCA2", "BRIP1", "RAD51C"))

# fwrite(as.data.table(beta_sub, keep.rownames = ""), paste0( "data/processed/methylation/","beta_for_plot.csv"), row.names = F, sep = "\t")
beta_sub <- fread("data/processed/methylation/beta_for_plot.csv", header = T, na.strings = "", fill = T) %>%
  dplyr::select(-V1)
...
right_join(meth_list) %>%
  ...
  geom_bar(stat = "identity")+
    # facet_wrap(~tumor_type, scale = "free", ncol = 8)+
  theme_bw()+
...
pdf("plots/Supplementary/Sup4_stage.pdf", width =14, height = 6)
...
pdf("plots/Supplementary/Sup4_age.pdf", width =14, height = 6)
demograph_table_meth %>%
  mutate(Dataset = factor(Dataset, levels = c("OV","TGCT", "STAD", "LGG", "BC", "SKCM", "UCEC", "GBM","LIHC","COAD", "HNSC","LUSC","THCA","PAAD","ESCA","CESC","KIRC", "BLCA","LUAD")),
         stage = gsub("Stage ", "", stage),
         stage = if_else(stage == "not reported", "NA", stage)) %>%
...
#Supplementary plot 7 - HRD predictors
```{r}
...
ggarrange(
  ...
  mutation_burden_plot[[1]]+theme(legend.position = "none", strip.text = element_blank(),
                 ...
                 plot.margin = margin(0,0,0,0, "lines"), panel.spacing = unit(0,"null"))
  ,nrow = 3, align = "v")
ggsave("plots/Supplementary/Sup7_mutation_burden.pdf", device = "pdf", height = 9, width = 12)
```
...
beta_sub_meth_mrna_t <- beta_sub_meth_t %>%
  ...
  left_join(mRNA%>% filter(Project == "OV") %>%
              dplyr::select(Sample.ID, BRCA1= ENSG00000012048, RAD51C = ENSG00000108384), by = c("Sample.ID")) %>%
