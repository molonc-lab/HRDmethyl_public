...
#Supplementary plot 7 - mutation burden
```{r}
stat.test <-mutation_burden %>%
  mutate(Methylation = factor(Methylation, levels = methylation_lvl),
         Project = factor(Project, levels = project_lvl),
         parameter_name = gsub("_", " ", parameter_name)) %>%
  filter(!is.na(value)) %>%
  ...
  mutate(y.position = case_when(parameter_name == "TMB nonsilent" ~ 100 + 5*row_number(),
                               parameter_name == "MSI Score"~ 35 + 5*row_number(),
                               parameter_name == "Aneuploidy Score"~ 30 + 5*row_number(),))   %>%
  ungroup() 
...
ggarrange(
   mutation_burden_plot[[3]]+
     stat_pvalue_manual(data = stat.test%>%
                          filter(parameter_name == "TMB nonsilent"), xmin ="group1", xmax = "group2",
    y.position = "y.position", label = "p.adj" )+
     ylim(c(0, 120))+
  ...
  mutation_burden_plot[[2]]+
         stat_pvalue_manual(data = stat.test%>%
                          filter(parameter_name == "MSI Score"), xmin ="group1", xmax = "group2",
    y.position = "y.position", label = "p.adj" )+
    ylab("MSI Score")+
    ylim(c(0, 52))+
  ...
  theme( strip.text = element_blank(),
        ...
        axis.text.y = element_text(hjust=1.2),strip.background = element_rect(fill = "white", colour = "black")),
  mutation_burden_plot[[1]]+
    ylab("Aneuploidy Score")+
    ylim(c(0, 50))+
    stat_pvalue_manual(data = stat.test%>%
                          filter(parameter_name == "Aneuploidy Score"), xmin ="group1", xmax = "group2",
    y.position = "y.position", label = "p.adj" )+
...
colnames(HRD_onco_mat) <- unlist(HRD_onco$"Sample.ID_original")
