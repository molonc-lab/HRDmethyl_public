...
#copy number 
copy_number_sample <- map_dfr(Sys.glob("data/raw/2022*/copynumber/TCGA_*cn_sample.tsv"),
                       ~ fread(.x, header = T)) %>%
...
beta_sub %>%
  filter(!is.na(PredefinedCpG)) %>%
  dplyr::select(CpG, PredefinedCpG) %>%
  distinct() %>%count(PredefinedCpG)
...
##demographic
```{r}
beta_sub_pd <- map_dfr(Sys.glob("data/raw/2022*/IDAT/*pd.csv"), ~ as.data.frame(fread(.x, header = T, na.strings = "", fill = T,  colClasses = c("Sentrix_ID"= "character")))) %>%
...
Deduplicated_beta %>%
  ...
  left_join( rbind(map_dfr(Sys.glob("data/raw/2022*/*clinical.tsv"), ~ as.data.frame(fread(.x, header = T, na.strings = "'--", fill = T,  colClasses = "character"))) %>%
...
write.csv( demograph_table_meth%>%
             ungroup()%>%
             mutate(Sample.ID = if_else(Dataset == "OV", DonorID, substr(SampleID,1,15))) %>%
            dplyr::select(Project =Dataset, Methylation, Sample.ID), "data/processed/TCGA_ICGC_methylation_list.tsv", quote = F, row.names = F)
```
...
##CN
```{r}
cnv <- Sys.glob("data/raw/2022*TCGA*meth/cn_seg/*.allelic_specific.seg.txt") %>%
...
BRCA1_cnv <- cnv %>%
  filter( (Start <= 43125041 & End >= 43125714) | (End >=43125041 & End <= 43125714)|
            (Start >=43125041 & Start <= 43125714)) %>%
  mutate(gene = "BRCA1")

RAD51C_cnv <- cnv %>%
  filter( (Start <= 58692529 & End >= 58693064) | (End >=58692529 & End <= 58693064)|
            (Start >=58692529 & Start <= 58693064)) %>%
  mutate(gene = "RAD51C")

cnv_sample <- Sys.glob("data/raw/2022*TCGA*meth/cn_seg/TCGA_*_cn_seg_sample.tsv")%>%
...
TCGA_cn <- cnv_sample %>%
  ...
  left_join(rbind(BRCA1_cnv, RAD51C_cnv)%>%
               mutate(LOH = if_else(Minor_Copy_Number ==0, "LOH", ".")) %>%
              dplyr::select(`File Name` = Sample, gene, CN = Copy_Number, LOH)) %>%
...
ICGC_cn <- fread("data/processed/copynumber/ICGC_OV_BRCA1_RAD51C_cnv.tsv") %>%
  ...
  mutate(Sample.ID  = paste(gsub("-", "_", CaseID),gsub("-", "_", Sample_Name),sep = "_"),
         V7 = if_else(str_detect(V7, "LOH")|V6 ==1, "LOH", ".")) %>%
  dplyr::select(Sample.ID, gene = V2, CN = V6, LOH = V7) %>%
  distinct()


##after manual assessment of the ASCAT output for BAF and LRR
## AOCS_094 & AOCS_107 are supported for LOH on the entire Chr17
## hence the LOH status are manually changed for these two sample
ICGC_cn <- ICGC_cn %>%
  mutate(LOH = if_else(Sample.ID %in% c("AOCS_094_ICGC_DBPC_20130205_067", "AOCS_107_ICGC_DBPC_20130607_004"), "LOH", LOH))
...
cn_purity <- beta_sub %>%
  filter(Sample_Name %in% Deduplicated_beta$Sample_Name) %>%
   left_join(rbind(ICGC_cn, TCGA_cn) %>%
               group_by(Sample.ID, gene) %>%
               slice(1), by = c("Sample.ID","gene")) %>%
  ...
  dplyr::select(gene, chr_start, CpG, distToTSS, CpG_context, Sample_Name, beta, Sample_Group, Project, Sample.ID,PredefinedCpG, CN, Sample.ID_original, purity,LOH )
...
hrd_score_tcga <- hrd_score %>%
  filter(Project != "OV") %>%
  dplyr::select(Project, Methylation ,Sample.ID, starts_with("HRD")) %>%
  mutate(Sample.ID = substr(Sample.ID, 1, 15)) %>%
  left_join(tcga_sig %>% 
              mutate(Sample.ID = substr(Sample, 1, 15)) %>%
              dplyr::select(-"Sample")) %>%
...
#methylation, a & b
 prevalence_plot <- beta_sub_meth_t %>%
  ...
  scale_y_continuous(labels = scales::percent, name = "Prevalence of Methylation", limits = c(0, 0.125)) +
...
#assemble plot
  ...
  ggsave("plots/Main_Figure_1a_Prevalence_of_RAD51C_BRCA1me.pdf", device = "pdf", width = 10, height = 6 )
```
...
ggsave("plots/Main_Figure_1b_Subtype_distribution.pdf",device = "pdf", height = 10,width = 11)
     
```

###Supplementary gene promoter rebuttal
```{r}

promoter <- fread("data/supporting/EDPnew_20240409.bed") %>%
  filter(str_detect(name, paste(hr_gene_list, collapse ="|"))) %>%
  mutate(gene = name) %>%
  separate(gene, into = c("Gene", "remove"), sep = "_") %>%
  dplyr::select(-remove)


annotation <- fread("data/supporting/HM450.hg38.manifest.gencode.v36.tsv.gz") %>%
  filter(str_detect(geneNames, paste(hr_gene_list, collapse ="|"))) %>%
  dplyr::select(CpG_chrm, CpG_beg, CpG_end, probeID, genesUniq, CGI, CGIposition) %>%
  mutate(Gene = case_when(str_detect(genesUniq, "BRCA1") ~ "BRCA1",
                          str_detect(genesUniq, "BRCA2") ~ "BRCA2",
                          str_detect(genesUniq, "RAD51C") ~ "RAD51C",
                          str_detect(genesUniq, "RAD51D") ~ "RAD51D",
                          str_detect(genesUniq, "RAD51B") ~ "RAD51B",
                          str_detect(genesUniq, "BRIP1") ~ "BRIP1",
                          str_detect(genesUniq, "PALB2") ~ "PALB2",
                          str_detect(genesUniq, "XRCC3") ~ "XRCC3")) %>%
  left_join(promoter %>%
  dplyr::select(CpG_chrm = `#chrom`, chromStart, chromEnd, name, Gene))


promoter_hypermethylation_probes <- annotation %>%
  separate(CGI, into = c("a", "b", "CGI_start", "CGI_end")) %>%
  filter(name != "BRCA1_2") %>%
  mutate(CGI_Promoter_overlap = if_else(chromStart<= CGI_end & CGI_start <= chromEnd, 1, 0),
    CGI = if_else(CGIposition=="Island" & !is.na(CGIposition), 1, 0),
         Promoter = if_else(CpG_beg >= chromStart &  CpG_end <= chromEnd, 1, 0),
         Promoter_200 = if_else(CpG_beg >= chromStart-200 & CpG_end <= chromEnd+200, 1, 0)) %>%
  group_by(probeID) %>%
  mutate(CGI_Promoter_overlap = sum(CGI_Promoter_overlap, na.rm = T ) >0,
         CGI = sum(CGI) >0,
         Promoter = sum(Promoter)>0, 
         Promoter_200 = sum(Promoter_200)>0) %>%
  dplyr::select(Gene, probeID, CpG_chrm, CpG_beg, CpG_end, CGI_Promoter_overlap, CGI, Promoter, Promoter_200)%>%
  distinct() %>%
  left_join(beta_sub %>%
              mutate(beta = beta) %>%
              filter( Sample_Group =="Primary Tumor") %>%
              dplyr::select(probeID = CpG, beta) %>%
              group_by(probeID)%>%
              summarise(IQR_Tumour = IQR(beta, na.rm = T), Median_Tumour = median(beta, na.rm = T))) %>%
    left_join(beta_sub %>%
              mutate(beta = beta) %>%
              filter( Sample_Group =="Solid Tissue Normal") %>%
              dplyr::select(probeID = CpG, beta) %>%
              group_by(probeID)%>%
              summarise(IQR_Normal = IQR(beta, na.rm = T), Median_Normal = median(beta, na.rm = T))) %>%
  mutate(threshold = IQR_Tumour <=0.2& Median_Tumour <=0.2 & IQR_Normal <= 0.2 & Median_Normal <= 0.2,
    INCLUDE = if_else((Promoter+Promoter_200 >0 | CGI_Promoter_overlap+CGI ==2) & threshold, TRUE, FALSE)) %>%
  ungroup() %>%
    na.omit() 


promoter_hypermethylation_probes$INCLUDE_ext = FALSE


for (i in seq(2, dim(promoter_hypermethylation_probes)[[1]])){
  if((promoter_hypermethylation_probes[i -1, "INCLUDE"]== TRUE &
     promoter_hypermethylation_probes[i , "threshold"] == TRUE)||
     (promoter_hypermethylation_probes[i -1, "INCLUDE_ext"]== TRUE &
     promoter_hypermethylation_probes[i , "threshold"] == TRUE)){
    promoter_hypermethylation_probes[i , "INCLUDE_ext"] <- TRUE
  }
}

for (i in seq(dim(promoter_hypermethylation_probes)[[1]]-1, 1)){
  if((promoter_hypermethylation_probes[i +1, "INCLUDE"]== TRUE &
     promoter_hypermethylation_probes[i , "threshold"] == TRUE) ||
     (promoter_hypermethylation_probes[i +1, "INCLUDE_ext"]== TRUE &
     promoter_hypermethylation_probes[i , "threshold"] == TRUE)){
...
promoter_hypermethylation_probes %>%
  write.table("plots/rebuttal/CpG_probes_annotation.tsv", quote = F,row.names = F, sep = "\t")

beta_sub %>%
  dplyr::select(probeID = CpG, Dataset = Project, Sample_Group, beta) %>%
  filter(Sample_Group %in% c("Primary Tumor", "Solid Tissue Normal")) %>%
  mutate(Sample_Group = if_else(Sample_Group == "Primary Tumor", "Tumour", "Normal"))%>%
           right_join(promoter_hypermethylation_probes %>%
                        dplyr::select(Gene, probeID, INCLUDE= INCLUDE_ext)) %>%
  group_by(Gene, probeID, Dataset , Sample_Group,INCLUDE) %>%
  summarise(IQR = IQR(beta), Median = median(beta)) %>%
  pivot_wider(values_from = c(IQR, Median), names_from = Sample_Group) %>%
  write.table("plots/rebuttal/CpG_probes_annotation_by_type.tsv", quote = F,row.names = F, sep = "\t")



CGI <- annotation %>%
  separate(CGI, into = c("type", "chrom", "start", "end")) %>%
  dplyr::select("Gene", "type", "chrom", "start", "end") %>%
...
col = c("CGI" = "blue", "Promoter" = "black", "Promoter+-200" = "black")
ggplot()+
  geom_rect(data = CGI, aes(xmax = start, xmin = end, ymin = 0, ymax = 1, color = "CGI"), fill = NA)+
  geom_rect(data = annotation %>%
              dplyr::select(Gene, chromStart, chromEnd, name) %>%
              distinct() %>%
              na.omit(), aes(xmax = chromStart, xmin=chromEnd, ymin = 0.08, ymax =0.92, color = "Promoter"),  linewidth = 0.4, fill = NA)+
  ...
  scale_color_manual(name = "", values = col,  guide = guide_legend(override.aes=aes(fill=NA)))+
  geom_point(data = beta_sub %>%
               filter(gene %in% hr_gene_list) %>%
               dplyr::select(Gene = gene, chr_start, chr_end) %>%
               distinct(), aes(x = chr_start, y = 0.5), size = 0.3, color = "orange")

ggsave("plots/rebuttal/CGI_promoter_relative_location.pdf", width = 16, height = 6)

ggplot()+
  geom_rect(data = CGI, aes(xmax = start, xmin = end, ymin = 0, ymax = 1, color = "CGI"), fill = NA)+
  geom_rect(data = annotation %>%
              dplyr::select(Gene, chromStart, chromEnd, name) %>%
              mutate(chromStart = chromStart-200, chromEnd = chromEnd + 200) %>%
              distinct() %>%
              na.omit(), aes(xmax = chromStart, xmin=chromEnd, ymin = 0.08, ymax =0.92, color = "Promoter+-200"),  linewidth = 0.4, fill = NA)+
  facet_wrap(~Gene, scales = "free_x")+
  theme_bw()+
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(),
        panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
...
ggsave("plots/rebuttal/CGI_promoter_ext_relative_location.pdf", width = 16, height = 6)



promoter_hypermethylation_probes%>%
  filter(INCLUDE_ext == TRUE) %>%
  group_by(Gene) %>%
  mutate(start_ext = min(CpG_beg), end_ext = max(CpG_end)) %>%
  ungroup()%>%
  filter(INCLUDE ==TRUE) %>%
  group_by(Gene) %>%
  mutate(start = min(CpG_beg), end = max(CpG_end),
         Gene = factor(Gene, levels= hr_gene_list)) %>%
  ungroup()%>%
  dplyr::select(Gene, start, end, start_ext, end_ext) %>%
  distinct()%>%
  ggplot() +
  geom_segment(aes(y = as.numeric(Gene)+0.2, yend =as.numeric(Gene)+0.2, x = start- start_ext, xend = end-start_ext, color = "Extended_Promoter+CGI"))+
  geom_text(aes(y = as.numeric(Gene)+0.2, x = end-start_ext + 80, label = paste0(end-start, " bps"),color = "Extended_Promoter+CGI"))+
  scale_y_continuous(labels = hr_gene_list, breaks = as.numeric(factor(hr_gene_list)))+
  geom_segment(aes(y = as.numeric(Gene)-0.2, yend =as.numeric(Gene)-0.2, x = 0 , xend = end_ext-start_ext, color = "Extension"))+
  geom_text(aes(y = as.numeric(Gene)-0.2, x = end_ext-start_ext + 80, label = paste0(end_ext-start_ext, " bps"), color = "Extension"))+
  theme_bw()+
  scale_color_manual(values = c( "Extended_Promoter+CGI" = "orange", "Extension" = "black"), "")+
  theme(legend.position = "bottom",panel.grid = element_blank(),
        axis.title = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank())+
  geom_point(data = promoter_hypermethylation_probes %>%
               dplyr::select(Gene, INCLUDE_ext,probeID, CpG_beg)%>%
               distinct()%>%
               filter(INCLUDE_ext==T)%>% group_by(Gene) %>%
  mutate(pos = CpG_beg- min(CpG_beg),
         Gene = factor(Gene, levels = hr_gene_list))%>%
    ungroup(), aes(y = as.numeric(Gene), x = pos), color = "black", shape =3 )

ggsave("plots/rebuttal/CpG_probe_extension_length.pdf", width = 8)
```
##Supplementary plot 1- HR gene promoter -- revised
```{r}
pdf("plots/Supplementary/Sup1_promoter_methylation_level.pdf",width = 10, height = 12)
beta_sub %>%
  filter(Sample_Group %in% c("Primary Tumor", "Solid Tissue Normal"), CpG %in% (promoter_hypermethylation_probes %>%
           filter(INCLUDE_ext == TRUE) %>%pull(probeID))) %>%
  group_by(Project, Sample_Group) %>%
  ...
  mutate(Sample_Group = factor(Sample_Group, levels= c("Primary Tumor", "Solid Tissue Normal")),
         ...
         gene = factor(gene, levels = c("BRCA1", "RAD51C", "BRIP1", "BRCA2", "PALB2", "RAD51B", "RAD51D", "XRCC3"))) %>%
  ...
  group_by(gene, Project, Sample_Name, Sample_Group, Sample_N) %>%
  summarise(avg_meth_level = median(beta)) %>%
  ...
  facet_grid(rows = vars(gene), cols = vars(Sample_N), scales = "free", space = "free", margins = F)+
  ylab("Median Methylation Level at Selected Promoter CpG Probes")+
  xlab("Dataset")+
...
```{r}
updated <- beta_sub %>%
  filter(Sample_Group %in% c("Primary Tumor"), CpG %in% (promoter_hypermethylation_probes %>%
           filter(INCLUDE_ext==TRUE) %>%pull(probeID)))  %>%
  group_by(gene, Sample.ID, Project) %>%
  mutate(probes = sum(round(beta,2) >=0.25, na.rm = T)  )%>%
  filter(probes >= round(n()*0.6,0))%>%
  ungroup() %>%
  dplyr::select(gene, Sample.ID, Project, probes) %>%
  distinct() %>%
  mutate(New= "Yes")
...
beta_sub %>%
  ...
  summarise(probes = round(n()*0.6,0), total_probes = n()) %>%
  ungroup()%>%
  dplyr::select(gene, probes, total_probes) %>%
  distinct()
...
beta_sub %>%
  filter(Sample.ID=="TCGA-AC-A2B8-01A", CpG %in% (promoter_hypermethylation_probes %>%
           filter(INCLUDE ==TRUE ||INCLUDE_ext==TRUE) %>%pull(probeID)), gene == "RAD51C") %>%
  mutate(number = sum(round(beta,2)>=0.25))


rbind(fread("/working/lab_olgak/lijunX/HRDmethyl_public/data/source_data/figure_1b.csv"), 
      fread("/working/lab_olgak/lijunX/HRDmethyl_public/data/source_data/figure_1a.csv"))%>%
  
  filter(Methylation != "No") %>%
  mutate(Old = "Yes") %>%
  full_join(updated ) %>%
  distinct() %>%
  mutate(Sample.ID = if_else(Project == "OV", substr(Sample.ID, 1, 8), substr(Sample.ID, 1,12))) %>%
  write.table("plots/rebuttal/changed_sample_status.csv", quote = F, sep = ",", row.names = F)

```

###duplicated sample rebuttal
```{r}
...
beta_sub_meth %>%
  na.omit()%>%
  filter(Sample_Group %in% c("Primary Tumor")) %>%
  mutate(CaseID = if_else(Project =="OV", substr(start = 1,stop = 8, Sample.ID), substr(start = 1,stop = 12, Sample.ID))) %>%
  group_by(CaseID, Project) %>%
  mutate(n_probes = n()) %>%
  ungroup() %>%
  filter(n_probes > mean(n_probes)) %>%
  dplyr::select(gene,Project, CpG, CaseID, Sample.ID, beta) %>%
  group_by(CaseID, CpG) %>%
  mutate(num = row_number()) %>%
  ungroup() %>%
  compare_means(beta ~ num, data = ., paired =T, group.by = c("Project,CpG"), method = "kruskal.test")
  group_by(CpG) %>%
  group_map(~  ggpaired(data= .x, x = "num", y = "beta", line.color = "gray", line.size = 0.4, facet.by = "CpG")+
   stat_compare_means(paired = TRUE), .keep = T)

suppressWarnings(duplicates)
```

##Supplementary plot 3- methylation heatmap 
```{r}
beta_sub_ht <- beta_sub %>%
  filter(CpG %in% (promoter_hypermethylation_probes %>%
           filter(INCLUDE_ext==TRUE, Gene %in% c("BRCA1", "RAD51C")) %>%
             pull(probeID)), Sample_Group == "Primary Tumor", Sample_Name %in% Deduplicated_beta$Sample_Name) %>%
  mutate(CaseID = if_else(Project =="OV", substr(start = 1,stop = 8, Sample.ID), substr(start = 1,stop = 12, Sample.ID))) %>%
  left_join(beta_sub_meth_t%>%
              mutate(CaseID = if_else(Project =="OV", substr(start = 1,stop = 8, Sample.ID), substr(start = 1,stop = 12, Sample.ID))) %>%
...
pdf("plots/Supplementary/Sup3_heatmap_methylation_NO_LEGEND.pdf")
# grid.newpage()
...
write.csv(beta_sub_meth %>%
  ...
  mutate(CaseID = if_else(Project == "OV", substr(Sample.ID, 1,8), substr(Sample.ID, 1, 15)),
         ...
         BRIP1me = if_else(CaseID %in% unlist(lapply(updated[which(updated$gene =="BRIP1" & updated$Project != "BC"),"Sample.ID"],function(x) substr(x, 1,15))), "Yes", "No")) %>%
  dplyr::select(CaseID, Dataset = Project, BRCA1me = BRCA1_M, RAD51Cme = RAD51C_M, BRCA2me, BRIP1me) %>%
  distinct(), "data/supplementary_data/Table_S1_methylation.csv", quote = F, row.names = F)

```
##Supplementary plot 2 BRCA2, PALB2 and BRIP1
```{r}
# RAD51B_meth <- beta_sub %>%
#   filter( Project %in% c("BC"), Sample_Group %in% c("Primary Tumor", "Solid Tissue Normal"), CpG %in% (promoter_hypermethylation_probes %>%
#            filter((INCLUDE_ext == TRUE ), Gene == "RAD51B") %>%pull(probeID))) %>%
#     mutate(CpG = factor(CpG, levels = unique(beta_sub$CpG[order(beta_sub$chr_start)])), gene2=gene) %>%
#   group_by(Sample_Name, Sample_Group) %>%
#   mutate(color = if_else(sum(beta >=0.25, na.rm = T) >= round(n()*0.6,0), "Methylated", "Unmethylated")
#          ) %>%
#   ungroup() %>%
...
#   scale_color_manual(values = c("Methylated" = "#D90368", "Unmethylated" = "grey"), "RAD51B")+
...
#                 scale_fill_manual("CpG context", values = c("Island" = "#DE2D26", "N_Shore" = "#FC9272", "S_Shore" = "#FEE0D2"),  guide = guide_legend(order = 2))+
#   xlab("Probes")
...
BRCA2_meth <- beta_sub %>%
  filter( Project %in% c("BC"), Sample_Group %in% c("Primary Tumor", "Solid Tissue Normal"), CpG %in% (promoter_hypermethylation_probes %>%
           filter((INCLUDE_ext == TRUE ), Gene == "BRCA2") %>%pull(probeID)))%>%
    mutate(CpG = factor(CpG, levels = unique(beta_sub$CpG[order(beta_sub$chr_start)])), gene2=gene) %>%
  group_by(Sample_Name, Sample_Group) %>%
  mutate(color = if_else(sum(beta >=0.25, na.rm = T) >= round(n()*0.6,0), "Methylated", "Unmethylated")) %>%
  ungroup() %>%
...
BRIP1_meth <- beta_sub %>%
  filter( Project %in% c("CESC", "LIHC", "UCEC", "STAD"), Sample_Group %in% c("Primary Tumor", "Solid Tissue Normal"),CpG %in% (promoter_hypermethylation_probes %>%
           filter((INCLUDE_ext == TRUE ), Gene == "BRIP1") %>%pull(probeID)))%>%
    mutate(CpG = factor(CpG, levels = unique(beta_sub$CpG[order(beta_sub$chr_start)])), gene2=gene) %>%
  group_by(Sample_Name, Sample_Group) %>%
  mutate(color = if_else(sum(beta >=0.25, na.rm = T) >= round(n()*0.6,0), "Methylated", "Unmethylated")) %>%
  ungroup() %>%
...
# ggsave(plot = PALB2_meth,filename = "plots/Supplementary/Sup2_PALB2me.pdf", height = 297/3*1.1, width = 200*1.1, units = "mm")
ggsave(plot = BRCA2_meth,filename = "plots/Supplementary/Sup2_BRCA2me.pdf", height = 297/3*1.1, width = 200*1.1, units = "mm")
ggsave(plot = BRIP1_meth,filename = "plots/Supplementary/Sup2_BRIP1me.pdf", height = 297/3*1.1, width = 200*1.1, units = "mm")
...
Molecular_subtype_table%<>%
  group_by(Project) %>%
  mutate(fisher.adj = p.adjust(fisher, method = "BH"))%>%
  ungroup()
...
stage <- demograph_table_meth %>%
  mutate(Dataset = factor(Dataset, levels = c("OV","TGCT", "STAD", "LGG", "BC", "SKCM", "UCEC", "GBM","LIHC","COAD", "HNSC","LUSC","THCA","PAAD","ESCA","CESC","KIRC", "BLCA","LUAD")),
         ...
         stage = case_when(stage %in% c('I', 'IA', 'IB', 'IS') ~ "I",
                           stage %in% c('II', 'IIA','IIB','IIC') ~ "II",
                           stage %in% c('III','IIIA','IIIB','IIIC') ~ "III",
                           stage %in% c('IV', 'IVA', 'IVB', 'IVC') ~ "IV",
                           TRUE ~ stage),
          Methylation = factor(Methylation, levels= c("BRCA1", "RAD51C", "BRCA1 & RAD51C", "No"))) %>%
  ...
  mutate(percent = n/total,
         stage = factor(stage, levels = c("I", "II", "III", "IV", 'X','NA'))) %>%
...
age_DF %>%
 ...
 geom_boxplot(data = . %>%
  ...
  scale_fill_manual(values = methylation_color2, "Promoter Methylated Gene")+
  scale_color_manual(values = methylation_color2, "Promoter Methylated Gene")+
 ...
 mutate(y.position = c(rep(c(95, 110),2),95, 95, 95,  95, 110, 90, 100, 110),
        p.adj = if_else(p.adj > 0.05 , "ns", if_else(Dataset == "OV", paste0("adj.p= ",as.character(round(p.adj, 3))), paste0("p= ",as.character(round(p.adj, 3)))))), label = "p.adj")+
  geom_text(data =. %>%
              dplyr::select(Dataset, Methylation, Methylation_count) %>%
              distinct(), mapping = aes(y = 0, label = Methylation_count)) +
  scale_x_discrete(limits = function(x) if(length(x) ==3 ){return(c("BRCA1", "RAD51C", "No"))} else if(length(x) == 4){return(c("BRCA1", "RAD51C", "BRCA1 & RAD51C", "No"))} else{ return(c("RAD51C", "No"))},
                   labels = function(x) str_wrap(x, width = 5))
  
ggsave(file="plots/Main_Figure_2_age_type.pdf",width = 300, height = 130, units = "mm")
...
ggsave(file="plots/Main_Figure_2_ances.pdf", width = 390,height = 100, units = "mm")

```
##Supplementary plot 4 - TNBC
```{r}
tnbc <- demograph_table_meth %>%
  ungroup() %>%
  filter(Dataset == "BC") %>%
  mutate(Sample.ID = substr(SampleID, 1 , 15)) %>%
  dplyr::select(-SampleID, -DonorID)%>%
  left_join(TCGA_subtype %>%
              filter(Project == "BC") %>%
              dplyr::select(Sample.ID, Subtype)) %>%
  filter(Subtype == "Basal") %>%
  dplyr::select(Dataset, Sample.ID, Subtype, consensus_ancestry, Methylation, age, stage)
...
tnbc %>%
  ...
  ungroup() %>%
  ggplot(aes(x = Methylation_count, fill= consensus_ancestry))+
...
ggsave("plots/Supplementary/Sup4_TBNC.pdf",width = 7, height = 7.5)
...
##Supplementary Table 5 - ancestry fisher
```{r}
ancestry_table <-  demograph_table_meth  %>%
  separate_rows(Methylation, sep = " & ") %>%
  mutate(Methylation = factor(Methylation, levels = c("BRCA1", "RAD51C", "No"))) %>%
  filter(consensus_ancestry != "NA") %>%
  dplyr::select(Dataset, DonorID, Methylation, consensus_ancestry) %>%
  distinct() %>%
  group_by(Dataset, Methylation, consensus_ancestry) %>%
  summarise(n=n()) %>%
  ungroup() %>%
  ...
  ungroup() %>%
  group_by(Dataset) %>%
  complete(consensus_ancestry, Methylation, fill = list(n=0)) %>%
  ungroup() %>%
  group_by(Dataset, consensus_ancestry) %>%
  mutate(consensus_ancestry_total = sum(n)) %>%
  ungroup() %>%
  group_by(Dataset, Methylation) %>%
  mutate(Methylation_total = sum(n)) %>%
  ungroup() %>%
  ...
  mutate(Cancer_total = sum(n)) %>%
  ungroup() %>%
  dplyr::select(Dataset, consensus_ancestry, Methylation, n) %>%
  distinct()
 
  

ancestry_fisher <- ancestry_table %>%
   group_by(Dataset) %>%
   group_map(~ fisher.test(x = t(matrix(unlist(.x$n),byrow = T, ncol = 3))))


 ancestry_table %>%
   dplyr::select(Dataset) %>%
   distinct() %>%
   mutate( ancestry_fisher_p= unlist(lapply(ancestry_fisher, function(x) x$p.value)))


write.csv( ancestry_table %>%
   dplyr::select(Dataset) %>%
   distinct() %>%
   mutate( ancestry_fisher_p= unlist(lapply(ancestry_fisher, function(x) x$p.value))), "data/supplementary_data/Table_S5_ancestry_fishers_molecular.csv", quote = F, row.names = F)
...
cimp_others <- TCGA_subtype %>%
  ...
  theme_bw()+
  scale_fill_manual(values = col$`CpG Methylation`, "CpG Methylation Cluster")+
  theme(legend.position = "bottom", axis.title.x = element_blank())+ 
  scale_y_continuous(label = scales::percent)+
  ylab("Percent of Patient")+
  scale_x_discrete(labels = function(x) str_wrap(x, width = 5))
...
##Supplementary Table 6 cimp fisher
```{r}
cimp_table <-  TCGA_subtype  %>%
  ...
  distinct() %>%
  mutate(Methylation = factor(Methylation, levels = c("BRCA1", "RAD51C", "No")),
         cimp_label = factor(cimp_label, levels = c("CIMP-",  "CIMPi","CIMP+"))) %>%
  arrange(Project, Methylation, cimp_label)
...
cimp_table %<>%
  group_by(Project) %>%
  filter(!(n_distinct(cimp_label)==2 & cimp_label=="CIMP-")) %>%
  group_by(Project) %>%
  mutate(fisher.adj = p.adjust(fisher, method = "BH"))

write.csv(cimp_table, "data/supplementary_data/Table_S6_cimp_fishers.csv", quote = F, row.names = F)
```
...
mut_beta_sub_meth_t_per_sample <- mut_beta_sub_meth_t %>%
  ...
  distinct() %>%
  filter( Hugo_Symbol %in% oncokb_gene_list$`Hugo Symbol`)
...
#   ungroup() %>%
#   mutate(pval.adj = p.adjust(fisher_p, method = "BH")) %>%
...
ggsave(plot = plot2 , "plots/Main_Figure_4_Mutation_prevalence_by_methylation.pdf", device = "pdf", width = 250, height = 120, units= "mm")

```

##Supplementary plot 6 co-mutation
```{r}
#by gene
...
gene_rad51c_prev <- grid.arrange(grobs = brca1_rad51c_unmeth_prev[2], nrow = 1,left = "Mutation Frequency in Unmethylated Sample",
             bottom = "Mutation Frequency in Methylated Sample")
plot2 <- ggarrange(arrangeGrob(gene_brca1_prev), arrangeGrob(gene_rad51c_prev))
ggsave(plot = plot2, filename = "plots/Supplementary/Sup5_gene_comutation.pdf", width = 250, height = 120, units= "mm" )
```

##Supplementary Table 7 Mutation fisher
```{r}
write_tsv(brca1_rad51c_unmeth_prev_table %>%
            filter(Methylated_gene == "BRCA1") %>%
             dplyr::select(Hugo_Symbol, Unmeth_Mut = Unmeth_mut, Meth_Mut = Meth_mut, Total_Unmeth = total_unmeth, Total_meth = total_meth, Prevalence_Unmethylated, Prevalence_Unmethylated, fisher_p, pval.adj, fisher_or), "data/supplementary_data/Table_S7_mutation_by_BRCA1.tsv")

write_tsv(brca1_rad51c_unmeth_prev_table %>%
            filter(Methylated_gene == "RAD51C") %>%
             dplyr::select(Hugo_Symbol, Unmeth_Mut = Unmeth_mut, Meth_Mut = Meth_mut, Total_Unmeth = total_unmeth, Total_meth = total_meth, Prevalence_Unmethylated, Prevalence_Unmethylated, fisher_p, pval.adj, fisher_or), "data/supplementary_data/Table_S7_mutation_by_RAD51C.tsv")

write_tsv(type_brca1_rad51c_prev_table %>%
            filter(Methylated_gene == "BRCA1") %>%
            dplyr::select(Dataset = Project, Hugo_Symbol, Unmeth_Mut = Unmeth_mut, Meth_Mut = Meth_mut, Total_Unmeth = total_unmeth, Total_meth = total_meth, Prevalence_Unmethylated, Prevalence_Unmethylated, fisher_p, pval.adj, fisher_or), "data/supplementary_data/Table_S7_mutation_by_project_BRCA1.tsv")

write_tsv(type_brca1_rad51c_prev_table %>%
            filter(Methylated_gene == "RAD51C") %>%
            dplyr::select(Dataset = Project, Hugo_Symbol, Unmeth_Mut = Unmeth_mut, Meth_Mut = Meth_mut, Total_Unmeth = total_unmeth, Total_meth = total_meth, Prevalence_Unmethylated, Prevalence_Unmethylated, fisher_p, pval.adj, fisher_or), "data/supplementary_data/Table_S7_mutation_by_project_RAD51C.tsv")
...
BRCA2_mRNA_table <- mRNA %>%
  ...
  mutate(Project = "BC",
         ...
         Methylation = if_else(Sample.ID %in% BRCA2_sample, "Yes", "No"),
          Methylation = factor(Methylation, levels = c("Yes", "No")),
         mRNA_norm_counts = round(mRNA_norm_counts,2))
...
BRIP1_sample <- updated%>%
  filter(gene=="BRIP1", Project != "BC")  %>%
  pull(Sample.ID)

BRIP1_mRNA_table <- rbind(mRNA%>%
  filter(Project %in% c("TCGA-LIHC", "TCGA-UCEC", "TCGA-STAD"), Sample_Group == "Primary Tumor") %>%
  ...
  mutate(Project = gsub("TCGA-", "", Project),
         ...
         Methylation = if_else(Sample.ID %in% BRIP1_sample, "Yes", "No"),
          Methylation = factor(Methylation, levels = c("Yes", "No"))) %>%
  mutate(mRNA_norm_counts = round(mRNA_norm_counts,2),
         )%>%
  distinct()
...
ggarrange(BRIP1_mRNA, BRCA2_mRNA, widths = c(0.7, 0.3), common.legend = F,  align = "v",ncol = 2, labels = c("c", "d"))
...
hrd_plot_df <- hrd_predictors   %>%
  ...
  separate_rows(Methylation, sep = " & ") %>%
   mutate(Methylation = case_when(Sample.ID %in% substr(updated[which(updated$gene =="BRCA2"),"Sample.ID"], 1,15) ~"BRCA2",
                                  Sample.ID %in% unlist(lapply(updated[which(updated$gene =="BRIP1" & updated$Project != "BC"),"Sample.ID"],function(x) substr(x, 1,15))) ~ "BRIP1",
                                  TRUE ~ Methylation)) %>%
  group_by(Project) %>%
...
stat.test <- hrd_plot_df %>%
  ...
  mutate(p.adj = p.adjust(p, method = "BH"),
         p.adj = case_when(p.adj >=0.05 ~ "ns",
                           p.adj <0.05 & p.adj >=0.0001 ~ as.character(round(p.adj, 4)),
                           p.adj <0.0001 ~ "<0.0001")) %>%
  ungroup() %>%
...
```
##Supplementary plot 7 HRD breakdown
```{r}
hrd_component_df <- hrd_plot_df %>%
  filter(Predictor == "HRD_Score") %>%
  dplyr::select(-starts_with("Sig"), -Predictor, -value) %>%
  pivot_longer(cols = c("HRD_TAI", "HRD_LST", "HRD_LOH"), names_to = "HRD component", values_to = "value") %>%
  mutate(`HRD component` = gsub("HRD_", "", `HRD component`)) %>%
  filter(!is.na(value))


hrd_component_stat <- hrd_component_df %>%
  group_by(Project, Methylation, `HRD component`, .keep = T) %>%
  filter(n() >2) %>%
  ungroup() %>%
  compare_means(value ~ Methylation, data =., group.by = c("Project", "HRD component"), ref.group = "No") %>%
  group_by(Project, HRD.component) %>%
  mutate(p.adj = p.adjust(p, method = "BH"),
         p.adj = case_when(p.adj >=0.05 ~ "ns",
                           p.adj <0.0001 ~ "<0.0001",
                          TRUE~ as.character(signif(p.adj, 2)))) %>%
  ungroup() %>%
  left_join(hrd_component_df %>%
              filter(Methylation != "No") %>%
              group_by(Project,  `HRD component`, .keep = T) %>%
              summarise(y.position = max(value, na.rm =T)) %>%
              ungroup() %>%
              dplyr::select(Project, HRD.component= `HRD component`, y.position)) %>%
  mutate(p.adj = if_else(p.adj >= 0.05, "ns", as.character(p.adj))) %>%
  group_by(Project, HRD.component) %>%
  mutate(y.position = if_else(HRD.component != "LST", 40-(40- y.position *1.1 )/n()*row_number(), 48-(40- y.position *0.7 )/n()*row_number()) )  %>%
  ungroup()


TAI_plot <- hrd_component_df %>%  
  filter(`HRD component` == "TAI") %>%
  ggplot(aes(x = Methylation, y = value, color =Methylation))+
  geom_point(position = position_jitterdodge(jitter.width = 0.8), size = 0.8)+
  geom_boxplot(outlier.shape = NA, data = . %>% ungroup()%>% group_by(Project, Methylation, .keep = T) %>%
  filter(n() >2) %>% ungroup(), alpha = 0)+
  ...
  theme(strip.text.y = element_blank(), axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(),
                    axis.text.y = element_text(hjust=1.2),strip.background = element_rect(fill = "white", colour = "black"))+
  facet_grid(cols = vars(Project), rows = vars(`HRD component`), scales="free", space = "free")+
    stat_pvalue_manual(hrd_component_stat %>% 
  filter(HRD.component == "TAI"), label = "p.adj", tip.length = 0)+
 scale_y_continuous("TAI", limits = c(0, 40))+
  scale_x_discrete(labels = function(x) str_wrap(x, width = 5))



LOH_plot <- hrd_component_df %>%
  filter(`HRD component` == "LOH") %>%
  ggplot(aes(x = Methylation, y = value, color =Methylation))+
  geom_point(position = position_jitterdodge(jitter.width = 0.8), size = 0.8)+
  geom_boxplot(outlier.shape = NA, data = . %>% ungroup()%>% group_by(Project, Methylation, .keep = T) %>%
  filter(n() >2) %>% ungroup(), alpha = 0)+
  ...
  facet_grid(cols = vars(Project), rows = vars(`HRD component`), scales= "free", space = "free")+
    stat_pvalue_manual(hrd_component_stat %>%
  filter(HRD.component == "LOH"), label = "p.adj", tip.length = 0)+
  scale_x_discrete(labels = function(x) str_wrap(x, width = 5))


LST_plot <- hrd_component_df %>%
  filter(`HRD component` == "LST") %>%
  ggplot(aes(x = Methylation, y = value, color =Methylation))+
  geom_point(position = position_jitterdodge(jitter.width = 0.8), size = 0.8)+
  geom_boxplot(outlier.shape = NA, data = . %>% ungroup()%>% group_by(Project, Methylation, .keep = T) %>%
  filter(n() >2) %>% ungroup(), alpha = 0)+
  ...
  filter(HRD.component == "LST"), label = "p.adj", tip.length = 0)+
  scale_x_discrete(labels = function(x) str_wrap(x, width = 5))

ggarrange(LOH_plot+theme(legend.position = "none"),
          TAI_plot+theme(legend.position = "none"),
          LST_plot+ theme(legend.position = "bottom"), heights = c(0.3,0.3,0.4), nrow = 3)
 

ggsave("plots/Supplementary/Sup6_HRD_breakdown.pdf", height = 10, width = 16)
```
##Supplementary Table 8 HRD feature stats
```{r}
write.csv(stat.test %>%
            filter(Predictor == "Sig3") %>%
             dplyr::select(Project, group2, pvalue =p, pvalue.adj = p.adj), "data/supplementary_data/Table_S8_Sig3.csv", quote = F, row.names = F)

write.csv(stat.test %>%
            filter(Predictor == "HRD_Score") %>%
             dplyr::select(Project, group2, pvalue =p, pvalue.adj = p.adj), "data/supplementary_data/Table_S8_HRD_Score.csv", quote = F, row.names = F)
...
HRD_onco <- beta_sub_meth_t%>%
  ...
  mutate(Sample.ID_original = if_else(Project =="OV", Sample.ID,substr(Sample.ID, 1, 15)),
    Sample.ID = if_else(Project =="OV", substr(Sample.ID, 1, 27),substr(Sample.ID, 1, 15))) %>%
  left_join(TCGA_subtype %>%
               dplyr::select(Sample.ID_original, cimp_label)) %>%
  left_join(mc3_gene %>%
              ungroup() %>%
              distinct() %>%
              mutate(Sample.ID = if_else(startsWith(Sample.ID, "AOC"), substr(Sample.ID, 1, 27), Sample.ID))) %>%
  ...
  dplyr::select(Project, Methylation, Sample.ID_original,DonorID, TP53, BRCA1, RAD51C, BRCA2, BRIP1, PALB2, HRD_Score, Sig3, cimp_label) %>%
  left_join(purity, by = c("Sample.ID_original" = "Sample.ID")) %>%
  left_join(beta_sub_meth %>%
              filter(CpG %in% (promoter_hypermethylation_probes %>%
           filter(INCLUDE_ext==TRUE) %>%pull(probeID)), Sample_Group =="Primary Tumor")%>%
              group_by(gene, Project, Sample.ID) %>%
              summarise(median_meth = median(beta)) %>%
  ...
  left_join(cn_purity_adj_beta %>%
              mutate(LOH = if_else(LOH ==".", "", LOH)) %>%
              group_by(gene, Sample.ID_original, CN, LOH) %>%
              summarise(median_corr_meth= median(mt_cap)) %>%
...
top_annotation =HeatmapAnnotation(
    ...
    `Median Meth` = anno_points(apply((matrix(HRD_onco_mat[c("median_meth","median_corr_meth"),],nc =2, byrow = T)),2, as.numeric),
...
lgd = Legend(labels = c("Raw", "Corrected"), legend_gp = gpar(col = c("#E69F00","#0072B2")), pch = c(21,3), type = "points", title = "Median Meth")
...
decorate_annotation("Median Meth", {
 grid.lines(seq(0.5, ncol(HRD_onco_mat)+3), c(0.7, 0.7), default.units = "native", gp = gpar(lty = 3,lwd=1))
})
dev.off()

```
###Supplementary Table 9 treatment inference
```{r}
sample_procurement <- Sys.glob("data/supporting/TCGA_treatment/nationwidechildrens.org_ssf_tumor_samples_*") %>%
  map_dfr(~ fread(.x), skip = 2) %>%
  dplyr::select(DonorID = bcr_patient_barcode, tumor_sample_procurement_days_to, tumor_sample_procurement_method, history_other_malignancy, history_neoadjuvant_treatment) %>%
  distinct()
sample_procurement <- sample_procurement[-c(1:2),]

write.csv(HRD_onco %>%
  filter(Project %in% c("BC", "UCEC", "OV")) %>%
   dplyr::select(Project, Methylation, Sample.ID_original, DonorID, purity, median_meth, median_corr_meth) %>%
   left_join(sample_procurement)%>%
    mutate(tumor_sample_procurement_method = gsub(",", "_", tumor_sample_procurement_method),
           history_other_malignancy = gsub(",", "_",history_other_malignancy)), "data/supporting/TCGA_treatment/summary_information_BC_UCEC.csv", quote = F, row.names = F)


 
```
## Supplemntary plot 5 -Survival
```{r}


survival <- rbind(fread("data/supporting/tcga_immunity.csv") %>%
  dplyr::select(CaseID = `TCGA Participant Barcode`, OS, OS.Time =`OS Time`, PFI, PFI.Time = `PFI Time`) %>%
    mutate(OS= if_else(OS == 1, 2, 1), PFI = if_else(PFI == 1, 2, 1)),
  fread("data/supporting/Patch_2015_ICGC_donor.tsv") %>%
    mutate(OS = if_else(donor_vital_status == "alive", 1, 2),
           OS.Time = donor_survival_time,
           submitted_donor_id = gsub("-", "_", submitted_donor_id),
          PFI =NA,
          PFI.Time = NA) %>%
    dplyr::select(CaseID = submitted_donor_id, OS, OS.Time, PFI, PFI.Time))
surv <-beta_sub_meth_t%>%
  mutate(Methylation= if_else(Methylation == "No", Methylation, gene)) %>%
  dplyr::select(-gene) %>%
  distinct() %>%
  group_by(Project, Sample.ID) %>%
  ...
  left_join(TCGA_subtype %>%
              mutate(CaseID= if_else(Project == "OV", substr(Sample.ID, 1, 8), substr(Sample.ID, 1, 12))) %>%
              dplyr::select(CaseID, Subtype, `Cancer Type Detailed`))
  

library("survival")
library("survminer")

pdf("plots/Supplementary/survival/BC_survival.pdf")
ggsurvplot(survfit(Surv(OS.Time, OS) ~  Methylation, data = surv %>%filter(Methylation %in% c("BRCA1", "RAD51C", "No"), Project =="BC", Subtype == "Basal" ) %>%  distinct() %>%
                                mutate(Methylation = factor(Methylation, levels = c("BRCA1", "RAD51C", "No")))), ylab = "Overall Survival", xlab = "Time (Days)", risk.table = TRUE, 
  ggtheme = theme_bw(), font.subtitle = 8 , font.caption = 2, risk.table.fontsize = 5,pval = T, pval.size = 4,risk.table.col="strata", palette = c("#93a667" , "#7BB2D9", "grey"), legend.labs =c("meBRCA1", "meRAD51C", "No") )
dev.off()


pdf("plots/Supplementary/survival/STAD_survival.pdf")
ggsurvplot(survfit(Surv(OS.Time, OS) ~  Methylation, data = surv %>%filter(Methylation %in% c("RAD51C", "No"), Project =="STAD", Subtype== "EBV") %>%  distinct() %>%
                                mutate(Methylation = factor(Methylation, levels = c("RAD51C", "No")))), ylab = "Overall Survival", xlab = "Time (Days)", risk.table = TRUE,
  ggtheme = theme_bw(), font.subtitle = 8 , font.caption = 2, risk.table.fontsize = 5,pval = T, pval.size = 4,risk.table.col="strata", palette = c( "#7BB2D9", "grey"), legend.labs =c( "meRAD51C", "No") )
dev.off()

pdf("plots/Supplementary/survival/OV_survival.pdf")
ggsurvplot(survfit(Surv(OS.Time, OS) ~  Methylation, data = surv %>%filter(Methylation %in% c("BRCA1","RAD51C", "No"), Project =="OV", Subtype == "HRD") %>%  distinct() %>%
                                mutate(Methylation = factor(Methylation, levels = c("BRCA1","RAD51C", "No")))), ylab = "Overall Survival", xlab = "Time (Days)", risk.table = TRUE,
  ggtheme = theme_bw(), font.subtitle = 8 , font.caption = 2, risk.table.fontsize = 5,pval = T, pval.size = 4,risk.table.col="strata", palette = c("#93a667", "#7BB2D9", "grey"), legend.labs =c("meBRCA1",  "meRAD51C", "No") )
dev.off()
...
ggsurvplot(survfit(Surv(OS.Time, OS) ~  Methylation, data = surv %>%filter(Methylation %in% c("BRCA1","RAD51C", "No"), Project =="OV", Subtype == "HRD") %>%  distinct() %>%
                                mutate(Methylation = factor(Methylation, levels = c("BRCA1", "No")))), ylab = "Overall Survival", xlab = "Time (Days)", risk.table = TRUE,
  ggtheme = theme_bw(), font.subtitle = 8 , font.caption = 2, risk.table.fontsize = 5,pval = T, pval.size = 4,risk.table.col="strata", palette = c("#93a667",  "grey"), legend.labs =c("meBRCA1",  "No") )

pdf("plots/Supplementary/survival/LGG_survival.pdf")
ggsurvplot(survfit(Surv(OS.Time, OS) ~  Methylation, data = surv %>%filter(Methylation %in% c("RAD51C", "No"), Project =="LGG") %>%  distinct() %>%
                                mutate(Methylation = factor(Methylation, levels = c("RAD51C", "No")))), ylab = "Overall Survival", xlab = "Time (Days)", risk.table = TRUE,
  ggtheme = theme_bw(), font.subtitle = 8 , font.caption = 2, risk.table.fontsize = 5,pval = T, pval.size = 4,risk.table.col="strata", palette = c( "#7BB2D9", "grey"), legend.labs =c( "meRAD51C", "No") )
dev.off()

pdf("plots/Supplementary/survival/TGCT_survival.pdf")
ggsurvplot(survfit(Surv(OS.Time, OS) ~  Methylation, data = surv %>%filter( Project =="TGCT", `Cancer Type Detailed` =="Mixed Germ Cell Tumor") %>%  distinct() %>%
                                mutate(Methylation = factor(Methylation, levels = c("BRCA1","RAD51C", "BRCA1 & RAD51C", "No")))), ylab = "Overall Survival", xlab = "Time (Days)", risk.table = TRUE,
  ggtheme = theme_bw(), font.subtitle = 8 , font.caption = 2, risk.table.fontsize = 5,pval = T, pval.size = 4,risk.table.col="strata", palette = c("#93a667", "#7BB2D9", "#46351D", "grey"), legend.labs =c("meBRCA1",  "meRAD51C", "meBRCA1 & meRAD51C ", "No") )
dev.off()


pdf("plots/Supplementary/survival/UCEC_survival.pdf")
ggsurvplot(survfit(Surv(OS.Time, OS) ~  Methylation, data = surv %>%filter( Project =="UCEC", Subtype =="CN HIGH") %>%  distinct() %>%
                                mutate(Methylation = factor(Methylation, levels = c("BRCA1", "No")))), ylab = "Overall Survival", xlab = "Time (Days)", risk.table = TRUE,
  ggtheme = theme_bw(), font.subtitle = 8 , font.caption = 2, risk.table.fontsize = 5,pval = T, pval.size = 4,risk.table.col="strata", palette = c("#93a667",  "grey"), legend.labs =c("meBRCA1",   "No") )
dev.off()
...
depmap_mRNA %>%
  ...
  ggsave("plots/Main_Figure_7c_depmap_expression.pdf", height = 3, width = 6)
```
##load data - depmap mutation release
```{r}
depmap_model <- fread("data/processed/depmap_brett/Model_23Q2_0523.csv") %>%
  filter(str_detect(CCLEName, "STOMACH"))

mutation_depmap <- fread(cmd = "grep -E 'TP53|PIK3CA|ARID1A|BRCA|RAD51|PALB2|BRIP1' data/processed/depmap_brett/OmicsSomaticMutations_23Q2_0523.csv") %>%
  `colnames<-`( colnames(fread(cmd = "head data/processed/depmap_brett/OmicsSomaticMutations_23Q2_0523.csv"))) %>%
...
depmap_hrd <- Sys.glob("../test/hrd_stad_array/*.txt") %>%
 ...
 depmap_loh <- read_xlsx("../test/CCLE_ABSOLUTE_COMBINED_20181227_segment_combined.xlsx") %>%
  filter(Chromosome == 17, Start <= 56769934, End >= 56812972) %>%
              filter(str_detect(sample, "_STOMACH")) %>%
              mutate(Sample.ID = gsub("_STOMACH", "", sample)) %>%
   dplyr::select(Sample.ID, LOH, Modal_Total_CN) %>%
   mutate(LOH = if_else(LOH == 0, "", "LOH"))
...
top_annotation =HeatmapAnnotation(
     ...
     `LOH\n(RAD51C)` = rad51c_depmap_mat["LOH",],
     `Copy Number\n(RAD51C)` = as.numeric(rad51c_depmap_mat["Modal_Total_CN",]),
     col = list(`RAD51Cme Status` = RAD51C_het_hom,
                `HRD Score`=HRD_color,
                `LOH\n(RAD51C)` = LOH,
                 `Copy Number\n(RAD51C)` = CN,
                CIMP = col$CIMP),gp = gpar(col = "white"),annotation_name_side = "left", annotation_name_rot = 0,
    annotation_name_gp = gpar(fontsize= 10), simple_anno_size = unit(0.8, "cm")))
    
pdf("plots/Main_Figure_7b_depmap_onco.pdf", height = 6, width =12)
...
rad51c_depmap %>%
  ...
  ggsave("plots/Main_Figure_7d_depmap_hrd.pdf", height = 3, width = 6)
```

##Main plot 7e Olap sensitivity -- Script adopoted from BrettL
This tunck was run under R/3.6.2
```{r}
#load data
olap_all_tall <- fread("data/processed/depmap_brett/Drug_sensitivity_replicate-level_dose_(CTD^2)_subsetted_NAsdropped.csv") %>%
  pivot_longer(starts_with("OLAPARIB"), names_to = "dose_rep", values_to = "CTD^2") %>%
  mutate(dose_rep = str_remove(dose_rep,"OLAPARIB [(]CTRP:411867[)] ")) %>%
  separate(dose_rep, sep = " ", into = c("dose","rep")) %>%
  mutate(dose_num = as.numeric(str_remove(dose, "μM")))


olap_all_tall_summary <- olap_all_tall %>%
  group_by(cell_line_display_name, dose_num, dose) %>%
  summarise(mean_CTD2 = mean(`CTD^2`, na.rm = TRUE),
            min_CTD2 = min(`CTD^2`, na.rm = TRUE),
            max_CTD2 = max(`CTD^2`, na.rm = TRUE)) %>%
  filter(dose > 0.5) %>%
  mutate(mean_CTD2 = if_else(mean_CTD2 > 1, 1, mean_CTD2)) %>%
  ungroup() %>%
  group_by(cell_line_display_name) %>%
  filter(min(mean_CTD2) < 1) %>%
  #remove celllines do not have RAD51C promoter info
  filter(!cell_line_display_name %in% c("KE39","KATOIII","MKN45", "IM95"))%>%
  mutate(group = case_when(cell_line_display_name == "SNU601" ~ "Homozygous RAD51Cme (SNU601)",
                       cell_line_display_name %in% c("AGS", "TGBC11TKB", "SNU620", "SNU719") ~ "Heterozygous RAD51Cme",
                       TRUE ~"No Methylation")) %>%
  left_join(depmap_hrd %>%
              dplyr::select(cell_line_display_name = Sample.ID, HRD= `HRD-sum`))%>%
  mutate(group = case_when(cell_line_display_name == "HGC27" ~ "HRD Score >= 42 & High sensitivity (HGC27)",
    HRD >=42 & group == "No Methylation" ~ "No Methylation (HRD Score >= 42)",
                           HRD <42 & group == "No Methylation" ~ "No Methylation (HRD Score < 42)",
                           TRUE ~group))


olap_cell_lines <- unique(olap_all_tall_summary$cell_line_display_name)

library("drc")
#fit viability curve
all_fits <- data.frame()
for( l in olap_cell_lines) {
  print(l)
  df_subset <- olap_all_tall_summary %>%  
    filter(cell_line_display_name == l)
  drm_model <- drm(data = df_subset,
                   mean_CTD2~dose_num, fct=LL.4(), 
                   )
                   #na.action = na.omit)
  fits <- data.frame(dose_num=exp(seq(log(min(df_subset$dose_num)),
                                 log(max(df_subset$dose_num)), 
                                 length=100)))
  temp_fit <- fits
  temp_fit$cell_line_display_name <- l
  temp_fit$p <- predict(drm_model, newdata=fits) 
  all_fits <- bind_rows(temp_fit,all_fits)
}

all_fits <- all_fits %>%
  left_join(olap_all_tall_summary %>% distinct(group, cell_line_display_name))%>%
  mutate(p = pmin(p, 1))



#plot
olab_plot <- olap_all_tall_summary %>%
  ggplot(aes(x=dose_num, y = mean_CTD2, colour = group, group = cell_line_display_name)) +
  geom_line(data = all_fits %>% filter(group == "HRD Score >= 42 & High sensitivity (HGC27)"), aes(y = p), size = 0.8, alpha = 0.5) +
  geom_point(data = .%>% filter(group == "HRD Score >= 42 & High sensitivity (HGC27)"), alpha = 0.5) +
  geom_line(data = all_fits %>% filter(group == "No Methylation (HRD Score < 42)"), aes(y = p), size = 0.8, alpha = 0.5) +
  geom_point(data = .%>% filter(group == "No Methylation (HRD Score < 42)"), alpha = 0.5) +
  geom_line(data = all_fits %>%filter(group == "No Methylation (HRD Score >= 42)"), aes(y = p), size = 0.8, alpha = 0.7) +
  geom_point(data = .%>% filter(group == "No Methylation (HRD Score >= 42)"), alpha = 0.7) +
  geom_line(data = all_fits %>%filter(group == "Heterozygous RAD51Cme"), aes(y = p), size = 1) +
  geom_point(data = .%>% filter(group == "Heterozygous RAD51Cme")) +
  geom_line(data = all_fits %>%filter(group == "Homozygous RAD51Cme (SNU601)"), aes(y = p), size = 1) +
  geom_point(data = .%>% filter(group == "Homozygous RAD51Cme")) +
  
  scale_colour_manual(values = c("Homozygous RAD51Cme (SNU601)" = "#165e91",
                                 "Heterozygous RAD51Cme" = "#7BB2DA",
                                 "HRD Score >= 42 & High sensitivity (HGC27)" = "orange",
                                 "No Methylation (HRD Score < 42)" = "lightgrey",
                                 "No Methylation (HRD Score >= 42)" = "#CEB1BE",
                                 "SNU520"= "black"), "") +
  xlim(1,150) +
  ylim(0.2,1.00) +
  scale_x_continuous(trans = log10_trans()) + 
  theme_bw() + 
  labs(x = expression("Olaparib Concentration ("*mu*"M)"),
       y = "Viability",
       colour = "Methylation") + 
    theme(
...
pdf("plots/Main_Figure_7_olaparib_STAD_dose_curve.pdf", width = 6, height = 6)
olab_plot
dev.off()
...
write.csv(age_DF, "data/source_data/figure_2a.csv", quote = F, row.names = F)
...
write.csv( demograph_table_meth %>%
  ...
  filter(n_distinct(Methylation)>1) %>%
  ungroup() %>%
    dplyr::select(Dataset, DonorID, sex, Methylation), "data/source_data/figure_2b.csv", quote = F, row.names = F)
...
write.csv(demograph_table_meth %>%
  ...
  ungroup() %>%
   group_by(Dataset) %>%
  filter(n_distinct(Methylation) >1  & n_distinct(stage )>1) , "data/source_data/figure_2d.csv", quote = F, row.names = F)
...
write.csv(HRD_onco %>%
            dplyr::select(-project_number, -number, -Sample.ID_original), "data/source_data/figure_6c.csv", quote = F, row.names = F)


write.csv(rad51c_depmap_mat, "data/source_data/figure_7b.csv", quote = F, row.names = F)
...
write.csv(rad51c_depmap %>%
            dplyr::select(Sample.ID, level, HRD_Score), "data/source_data/figure_7d.csv", quote = F, row.names = F)

write.csv(olap_all_tall_summary, "data/source_data/figure_7e_points.csv", quote = F, row.names = F)
write.csv(all_fits, "data/source_data/figure_7e_curves.csv", quote = F, row.names = F)


```
#Source data Supp Figure
```{r}
write.csv(beta_sub %>%filter(Sample_Group %in% c("Primary Tumor", "Solid Tissue Normal"),  gene_promo_200 != "CHEK2") %>%
                               mutate(Sample.ID = if_else(Project == "OV", substr(Sample.ID, 1, 8), Sample.ID)) %>%
            dplyr::select(Dataset = Project, gene, CpG, Sample.ID, Sample_Group, beta),
...
write.csv(tnbc, "data/source_data/supplementary_figure_3.csv", quote = F, row.names = F)

write.csv(beta_sub %>%
  filter( Project %in% c("CESC", "LIHC", "UCEC"), Sample_Group %in% c("Primary Tumor", "Solid Tissue Normal"), gene_promo_200 == "BRIP1") %>%
    mutate(CpG = factor(CpG, levels = unique(beta_sub$CpG[order(beta_sub$chr_start)])), gene2=gene) , "data/source_data/supplementary_figure_4a.csv", quote = F, row.names = F)

write.csv(beta_sub %>%
  filter( Project %in% c("BC"), Sample_Group %in% c("Primary Tumor", "Solid Tissue Normal"), gene_promo_200 == "BRCA2")%>%
    mutate(CpG = factor(CpG, levels = unique(beta_sub$CpG[order(beta_sub$chr_start)])), gene2=gene) , "data/source_data/supplementary_figure_4b.csv", quote = F, row.names = F)

write.csv(beta_sub %>%
  filter( Project %in% c("LUSC"), Sample_Group %in% c("Primary Tumor", "Solid Tissue Normal"), gene_promo_200 == "PALB2") %>%
    mutate(CpG = factor(CpG, levels = unique(beta_sub$CpG[order(beta_sub$chr_start)])), gene2=gene) , "data/source_data/supplementary_figure_4c.csv", quote = F, row.names = F)


write.csv(brca1_rad51c_unmeth_prev_table %>%
            dplyr::select(-pval.adj_plot, - .keep), "data/source_data/supplementary_figure_5.csv", quote = F, row.names = F)

write.csv(hrd_component_df %>%
            dplyr::select(Project, Methylation, Sample.ID, `HRD component`, value), "data/source_data/supplementary_figure_6.csv", quote = F, row.names = F)



write.csv(explore_df %>%
            dplyr::select(Project, Methylation, Sample.ID, purity, HRD_Score, avg_meth), "data/source_data/supplementary_figure_7a.csv", quote = F, row.names = F)

write.csv(explore_df %>%
            dplyr::select(Project, Methylation, Sample.ID, purity, HRD_Score, avg_meth, zscore), "data/source_data/supplementary_figure_7b.csv", quote = F, row.names = F)
...
##gLOH
```{r}
arm_len <- fread("data/supporting/cytoBand.txt") %>%
  mutate(arm = substr(V4, 1,1 ),chrom = V1) %>%
  group_by(chrom, arm) %>%
  summarise(arm_length = sum(V3-V2)) %>%
  ungroup() %>%
  mutate(chrom = gsub("chr", "", chrom)) %>%
  group_by(chrom) %>%
  mutate(chr_length = sum(arm_length)) %>%
  ungroup() %>%
  mutate(arm_start = if_else(arm == "p", 0, chr_length - arm_length))


gLOH_tcga <- fread("data/supporting/TCGA_mastercalls.abs_segtabs.fixed.txt") %>%
  dplyr::select(Sample.ID_short = Sample, chrom = Chromosome, start = Start, end = End, segment.length = Length, LOH) %>%
  mutate(LOH = if_else(LOH == 0, FALSE, TRUE),
         chrom = as.character(chrom))%>%
  left_join(arm_len %>%
              filter(arm == "p") %>%
              dplyr::select(chrom, arm_length), by = "chrom") %>%
  mutate(arm = if_else(start > arm_length, "q", "p")) %>%
  dplyr::select(-arm_length) %>%
  left_join(arm_len) %>%
  filter(!chrom %in% c("X", "Y") ) %>%
  mutate(large_scale = case_when( segment.length >= 0.9 *chr_length ~ TRUE,
                                  arm == "q" & start >= arm_start - arm_length*0.1 & LOH == TRUE & segment.length >= 0.9 *arm_length ~ TRUE,
                                  arm == "p" & LOH == TRUE & segment.length  >= 0.9 * arm_length & start <= arm_length * 0.1 ~ TRUE,
                                 TRUE ~FALSE))%>%
  filter(large_scale == FALSE) %>%
  group_by(Sample.ID_short , LOH) %>%
  summarise(sum_seg_len = sum(segment.length)) %>%
  ungroup() %>%
  group_by(Sample.ID_short) %>%
  mutate(gLOH = sum_seg_len/sum(sum_seg_len)) %>%
  ungroup() %>%
  filter(LOH == TRUE) %>%
  dplyr::select(Sample.ID_short , gLOH)

  
gLOH_icgc <-Sys.glob("data/raw/ascat/AOCS*.copynumber.ascat.tsv") %>%
  map_dfr( ~ fread(.x) %>%
             mutate(Sample.ID = gsub("-", "_",substr(basename(.x), 1, 31)),
               Sample.ID_merge = gsub("-", "",gsub("_", "", Sample.ID)))) %>%
  filter(Sample.ID_merge %in% primary_tumour_ICGC$Sample.ID_merge) %>%
  dplyr::select(chrom = chromosome, start, end, segment.length, Sample.ID, LOH) %>%
  left_join(arm_len %>%
              filter(arm == "p") %>%
              dplyr::select(chrom, arm_length), by = "chrom") %>%
  mutate(arm = if_else(start > arm_length, "q", "p")) %>%
  ...
  filter(LOH == TRUE) %>%
  mutate(Sample.ID =substr(Sample.ID, 1, 8)) %>%
  dplyr::select(Sample.ID_short = Sample.ID, gLOH)


gLOH <- hrd_plot_df %>%
  dplyr::select(Project, Sample.ID, Methylation) %>%
  distinct()%>%
  mutate(Sample.ID_short = if_else(Project == "OV", substr(Sample.ID,1,8), substr(Sample.ID,1,15))) %>%
  left_join(rbind(gLOH_tcga, gLOH_icgc)) %>%
  filter(!is.na(gLOH)) %>%
  group_by(Project, Methylation) %>%
  mutate(Methylation_N = paste0("N=", n())) %>%
  ungroup()
 ...
 gLOH_stat <- gLOH %>%
  group_by(Project, Methylation, .keep = T) %>%
  filter(n() >2) %>%
  ungroup() %>%
  compare_means(gLOH ~ Methylation, data =., group.by = c("Project"), ref.group = "No") %>%
  left_join(gLOH %>%
              group_by(Project,  .keep = T) %>%
              summarise(y.position = max(gLOH, na.rm =T)) %>%
              ungroup() %>%
              dplyr::select(Project, y.position)) %>%
  mutate(p.adj = if_else(p.adj >= 0.05, "ns", as.character(p.adj))) %>%
  group_by(Project) %>%
  mutate(y.position = 1-(1- 1.1 * y.position)/n()*row_number())   %>%
  ungroup()
...
gLOH %>%  
  ...
  filter(n_distinct(Methylation) >1) %>%
  ungroup() %>%
  ggplot(aes(x = Methylation, y = gLOH, color =Methylation))+
   geom_text(data = .%>%
              dplyr::select(Project, Methylation, Methylation_N) %>%
              distinct(), mapping = aes( y =-0.1, label = Methylation_N), color = "black", size = 2.5)+
  geom_point(position = position_jitterdodge(jitter.width = 0.8), size = 0.8)+
  geom_boxplot(outlier.shape = NA, data = . %>% ungroup()%>% group_by(Project, Methylation, .keep = T) %>%
  filter(n() >2) %>% ungroup(), alpha = 0)+
  theme_bw()+
  ...
  theme(strip.text.y = element_blank(), axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(),
                    axis.text.y = element_text(hjust=1.2),strip.background = element_rect(fill = "white", colour = "black"))+
  facet_grid(cols = vars(Project), scales="free", space = "free")+
    stat_pvalue_manual(gLOH_stat , label = "p.adj", tip.length = 0)+
 # scale_y_continuous("TAI", limits = c(0, 38))+
  scale_x_discrete(labels = function(x) str_wrap(x, width = 5))+
  theme(legend.position = "bottom")+
  scale_y_continuous(labels = scales::percent)+
...
ggsave("plots/Supplementary/Sup7_hrd_regression.pdf", height =4, width = 8)
...
methylation_list <- fread("data/processed/TCGA_ICGC_methylation_list.tsv") %>%
  filter(Project %in% c("BC", "UCEC")) %>%
  mutate(Sample.ID = substr(Sample.ID, 1, 12)) %>%
  filter(Methylation != "No")

treatment <- Sys.glob("data/supporting/TCGA_treatment/*.xml") %>%
  map_dfr(~ xmlToDataFrame(xmlParse(.x)))

treatment_meth <- treatment %>%
  dplyr::select(bcr_patient_barcode, history_of_neoadjuvant_treatment, drugs) %>%
  distinct() 

%>%
  filter(bcr_patient_barcode %in% methylation_list$Sample.ID)

treatment_meth %>%
  count(history_of_neoadjuvant_treatment)
...
# export ICGC HM450 for igv
```{r}
library(rtracklayer)
ch = import.chain("/working/lab_olgak/lijunX/OV_meBRCA1_Nanopore/ref/hg38ToHg19.over.chain")

beta_hg38 <- beta_sub_meth %>%
  filter(Project == "OV",gene == "BRCA1",startsWith(Sample.ID, "AOCS_158"), PredefinedCpG=="BRCA1")%>%
  mutate(chr = "chr17", name = "beta_hg19", beta = round(beta * 1000),
         chr_start = 41277187) %>%
  dplyr::select(chr, start = chr_start, end = chr_end, name, beta, strand =probe_strand) 
beta_hg38 <- makeGRangesFromDataFrame(beta_hg38, keep.extra.columns = T) 
seqlevelsStyle(beta_hg38) = "UCSC" 


beta_hg19<- liftOver(beta_hg38, ch)
data.frame(beta_hg19) %>%
  dplyr::select(seqnames, start, end , name, beta, strand) %>%
  write.table(., file = "/working/lab_olgak/lijunX/OV_meBRCA1_Nanopore/AOCS_158_brca1.bed", quote=F, sep="\t", row.names=F, col.names=F)
```