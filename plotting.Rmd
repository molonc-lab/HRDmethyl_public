...
olap_all_tall_summary <- olap_all_tall %>%
  ...
  mutate(group = case_when(cell_line_display_name == "SNU601" ~ "Homozygous RAD51Cme (SNU620)",
...
olab_plot <- olap_all_tall_summary %>%
  ...
  geom_line(data = all_fits %>%filter(group == "Homozygous RAD51Cme (SNU620)"), aes(y = p), size = 1) +
  geom_point(data = .%>% filter(group == "Homozygous RAD51Cme")) +
  
  scale_colour_manual(values = c("Homozygous RAD51Cme (SNU620)" = "#165e91",
