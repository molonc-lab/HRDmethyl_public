...
```
```{r load files}
beta_norm <- fread("data/processed/methylation/ICGC_OV_beta_subset_promo.csv")
...
methylation_status <-fread(methylation_status) %>%
  filter(startsWith( Sample.ID,"AOCS")) %>%
...
#HRDetect
icgc_hrdetect <- fread("tables/hrdetect.tsv") %>%
  mutate(Sample.ID = gsub("-","_", Tumor_Sample)) %>%
  dplyr::select(-Tumor_Sample)


ov_groups <-read_excel("data/supporting/nature14410-Supplementary Table 13.xlsx", sheet = 2, skip = 2) %>%
...
pdf("plots/Supplementary/Sup8b_silhouette.pdf")
...
as.data.frame(cpg_umap) %>%
  ...
  ggplot(aes(x= UMAP1, y =UMAP2,fill = Cluster, color = Cluster, shape = Methylation))+
  geom_point(size = 4)+
...
onco <- oncoPrint(
  ...
  top_annotation =HeatmapAnnotation(
   ...
   `Avg Meth\n(BRCA1)` = anno_points(apply((matrix(ov_mut_meth_matrix[c("BRCA1_mean_beta","BRCA1_mean_adj_beta"),], nc =2, byrow = T)),2, as.numeric), pch = c(21,3),gp = gpar(col = "#93a667"),show_annotation_name = F, axis_param = list(at = c(0, 0.5, 1) )),
   `CN(BRCA1)` = as.numeric(ov_mut_meth_matrix["BRCA1_CN",]),
    `Avg Meth\n(RAD51C)` = anno_points(apply((matrix(ov_mut_meth_matrix[c("RAD51C_mean_beta","RAD51C_mean_adj_beta"),],nc =2, byrow = T)),2, as.numeric), pch = c(21,3),gp = gpar(col = "#7BB2D9"),show_annotation_name = F, axis_param = list(at = c(0, 0.5, 1))),
