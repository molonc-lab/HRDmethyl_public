...
pvalue_filter <- pd %>%
  filter(Sample_Group == "Primary Tumour", Sample.ID %in% Deduplicated_beta$Sample.ID) %>%
...
colnames(beta_norm_corr) <- unlist(lapply(colnames(beta_norm_corr), function(x) unlist(strsplit(x, split = "_"))[1]))


beta_matrix <-  data.frame(beta_norm_corr) %>% 
   dplyr::select( pvalue_filter$Sample_Name) %>%
