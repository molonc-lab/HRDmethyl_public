...
#re-run signature
```{r run_signature_TCGA_MAF}
#read meta file to match patient id
tcga_snp_meta <- Sys.glob("/working/lab_nicw/lijunX/20220401_TCGA_*_meth/MAF/*.tsv")%>%
  map_dfr(~ as.data.frame(fread(.x)))%>%
...
#read snv maf
tcga_snp <- Sys.glob("/working/lab_nicw/lijunX/20220401_TCGA_*_meth/MAF/*.maf") %>%
...
icgc_snp <- Sys.glob("/working/lab_nicw/lijunX/20220505_ICGC_*_meth/MAFs/*.shc.maf.gz") %>%
...
cosmic_t <- read_delim("/working/lab_olgak/lijunX/20230314_PDX_SFRC01217/hrdetect/data/supporting/signatures_probabilities.txt",
                      delim = "\t") 
cosmic_t %<>%
...
write_tsv(tcga_somatic_snp_cnts, "./tables/tcga_somatic_snp_cnt.tsv")
...
write_tsv(icgc_somatic_snp_cnts, "./tables/icgc_somatic_snp_cnt.tsv")
...
cat("##fileformat=VCFv4.1\n##contig=<ID=1,length=249250621,assembly=b37>\n##reference=file:///path/to/human_g1k_v37.fasta\n",
    append=FALSE, 
    file="/working/lab_nicw/lijunX/vep_testing/HR_TP53_variants_tcga_icgc_hg19.vcf")


HR_tp53_vcf <- rbind(mc3 %>% 
