...
beta_p_value = "../data/processed/methylation/TCGA_TGCT_beta_pvalue.csv"
clinical_csv = "../data/raw/20220401_TCGA_TGCT_meth/IDAT/TCGA_TGCT_pd.csv"
purity_csv = "../data/processed/TCGA_ABSOLUTE_purity.txt"
demographic_csv = "../data/raw/20220401_TCGA_TGCT_meth/tgct_clinical.tsv"
...
write.csv(indS_clipped_age_filtered, "../data/processed/methylation_clustering/TGCT_preprocessed_beta_matrix_for_clustering.csv", row.names = T)
```


#summary
```{r}
filter_list <- as.data.frame(fread(paste0("gunzip -cq ",hm450_annotation ))) %>%
  mutate(filter =case_when(is.na(CGI) ~"not CGI associated",
                           CpG_chrm %in% c("chrX", "chrY") ~ "sex related",
                           probeID %in% filter_variable_cgi[which( filter_variable_cgi$cluster != high_variable_cgi$cluster), "V1"] ~ "low variable",
                           probeID %in% age_correlated_cpg ~ "age correlated")) %>%
  filter(!is.na(filter)) %>%
  dplyr::select(probeID, filter)

write.csv(filter_list, "../data/processed/methylation_clustering/TCGT_filtered_probes", row.names = F, quote = F)
```
...
meth_data_pc = []
meth_data_pc =pd.read_csv("data/processed/methylation_clustering/TGCT_preprocessed_beta_matrix_for_clustering.csv", index_col=0)
...
new_meth_data_pc = {}
new_meth_data_pc["TGCT"] =  meth_data_pc
...
consensus_matrix_spectral, affinity_matrix = get_consensus_matrix_clustering(new_meth_data_pc, ["TGCT"], [],1,"NN")
...
kw_snf = get_sign_kw_louvain(new_meth_data_pc, ["TGCT"], consensus_clustering_spectral)
